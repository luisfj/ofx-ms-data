...
public class OperacaoService {
      ...
      public List<OperacaoEntity> importarOperacoes(Long idUser, Long idUe, ImportacaoEntity importacao,
                  List<Operation> operacoes) {
            AtomicInteger atomicInteger = new AtomicInteger(0);
            var operacoesPersist = operacoes
                        .stream()
                        .sorted(Comparator.comparing(Operation::dt))
                        .map(op -> new OperacaoEntity(null, idUser, idUe, importacao.getId(),
      ...
      public Mono<List<OperacoesDTO>> findByIdImportacao(Long idUser, Long idUe, Long idImportacao) {
            return asyncRepository.findByIdImportacao(idUser, idUe, idImportacao).collectList()
                        .map(this::organizaGruposEFilhos);
      }

      public Flux<OperacoesDTO> findOperacoesPendendesByDataBetween(Long idUser, Long idUe, LocalDate dtInicial,
                  LocalDate dtFinal) {
            return asyncRepository.findOperacoesPendendesByDataBetween(idUser, idUe, dtInicial, dtFinal);
      }
      ...
      public void atualizarOperacoes(Long idUser, Long idUe, UpdateOperacoesDTO operacoesDTO) {
      ...
      public NovaOperacaoResponseDTO salvarOperacao(Long idUser, Long idUe, NovaOperacaoDTO novaOperacaoDTO) {
            ...
            operacaoEntity.setTipo(BigDecimal.ZERO.compareTo(novaOperacaoDTO.valor()) >= 0 ? TipoOperacao.CREDIT
                        : TipoOperacao.DEBIT);
            operacaoEntity.setIdUser(idUser);
      ...
      private OperacaoEntity findOperacaoByIdAndIdUserAndIdUe(Long idUser, Long idUe, Long idOperacao) {
            return repository.findByIdAndIdUserAndIdUe(idOperacao, idUser, idUe)
                        .orElseThrow(() -> new RuntimeException(
                                    String.format("Operação com id:%s não existe na base de dados", idOperacao)));

      }

      public NovaOperacaoResponseDTO atualizarOperacao(Long idUser, Long idUe, Long idOperacao,
                  NovaOperacaoDTO novaOperacaoDTO) {
            var operacaoEntity = findOperacaoByIdAndIdUserAndIdUe(idUser, idUe, idOperacao);
      ...
      public Mono<NovaOperacaoResponseDTO> buscarOperacaoPorId(Long idUser, Long idUe, Long idOperacao) {
            return asyncRepository.findByIdUserAndIdUeAndId(idUser, idUe, idOperacao)
                        .map(OperacaoMapper.INSTANCE::entityToNovaOperacaoResponseDTo);
      }

      public void excluirOperacaoDefinitivamente(Long idUser, Long idUe, Long idOperacao) {
            var operacaoEntity = findOperacaoByIdAndIdUserAndIdUe(idUser, idUe, idOperacao);
            repository.delete(operacaoEntity);
      }

      public void removerOperacaoDoGrupo(Long idUser, Long idUe, Long idOperacao) {
            var operacaoEntity = findOperacaoByIdAndIdUserAndIdUe(idUser, idUe, idOperacao);
            operacaoEntity.setIdGrupo(null);
            repository.save(operacaoEntity);
      }

      public void agruparOperacoes(Long idUser, Long idUe, Long idGrupo, List<Long> idOperacoes) {
            var operacoes = repository.findAllByIdIn(idOperacoes);
            if (operacoes.stream().anyMatch(op -> Objects.nonNull(op.getIdGrupo())
                        || !idUser.equals(op.getIdUser())
